...
coroutine_t sched_current_coroutine(sched_t sched)
{
    return sched->co_curr;
}

int autoremove_wake_function(wait_queue_t *wait, int wake_flags)
{
    int ret = sched_wakeup_coroutine(wait, wake_flags);
...
int sched_wakeup_coroutine(wait_queue_t *curr, int wake_flags)
{
    struct coroutine *co = (struct coroutine*)curr->private;
